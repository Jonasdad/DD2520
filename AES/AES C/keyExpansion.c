...
void transform(uint8_t word[4], uint8_t rcon_index);
void main(){
    ...
    uint8_t calc_word[4];
    memcpy(calc_word, word_3, 4);
    //Create new first word of key:
//    printf("\n");
    for(int j = 1; j < 11; j++){
        RotWord(calc_word);
        SubWord(calc_word);
        calc_word[0] ^= rcon[j-1];
        for(int i = 0; i < 4; i++){   
            keys[j][i] = keys[j-1][i] ^ calc_word[i];
//            printf("%x ", keys[j][i]);
        }
//        printf("\n");
        for(int k = 4; k < 16; k++){
